...
public class CopyOfORControl {
	
  private static final String CONFIG_FILE = "src/house.xml";
  private static Config config = new Config(CONFIG_FILE);
	
  // Maximum number of devices
  private static final int MAX_SENSORS = Config.MAX_SENSORS;
  private static final int MAX_IAMS = Config.MAX_APPLIANCES;
  private static final int MAX_SOCKETS = Config.MAX_SOCKETS;
  private static final int MAX_LIGHTS = Config.MAX_LIGHTS;
  private static final int MAX_WINDOWS = Config.MAX_SWITCHES;
  
  private static final int LIVING_ROOM = 0; 
  
  // Room nodes for house temperature and humidity
  private static final int HEATING_TEMPERATURE_SENSOR = config.temperatureSensor; 
  private static final int PLANT_HUMIDITY_SENSOR = config.plantSensorId;
	
  // Ports for transceivers
  private static final String RFM12_PORT = config.rfm12Port;
  private static final String LWRF_PORT = config.lwrfPort;
  private static final String IAM_PORT = config.iamPort;
  
  // Set the voice to Kevin
  private static boolean localSpeech = config.speechLocal;
  private static String VOICE = config.speechVoice;
	
  /// Email data
  private static boolean email = config.email;
  private static final String EMAIL_USER = config.emailUser;
  private static final String EMAIL_PASSWORD = config.emailPassword;
  private static final String EMAIL_TARGET = config.emailRecipient;
  
  // OpenEnergyMonitor data
  private static final int EMONTX_NODE = config.getEmontxId();
  // Minimum emontx battery level in millivolts
  private static int BATTERY_LOW_LEVEL = config.emontxBatteryLow;
  
  // EDF IAM data
  static final int IAM_MIN_VALUE = config.iamMinValue;
  static final int IAM_MIN_OFF_PERIOD = config.iamMinOffPeriod;
  // EDF IAM IDs
  private static long[] iamIds = config.iamCodes;
  
  // COSM data
  private boolean cosm = config.cosm; // set true to post energy data to COSM
  private static final String COSM_API_KEY = config.cosmApiKey;
  private static final int COSM_FEED = Integer.parseInt(config.cosmFeed);
  private static String COSM_POWER_STREAM = config.cosmPower;
  private static String COSM_ENERGY_STREAM = config.cosmEnergy;
  
  // The name of the log4j2 logger used by this class
  private static final String LOGGER_NAME = "ORControl";
  
  // Mobile phone data
  private static final String MOBILE_PHONE_HOST_NAME = config.phoneName;
  
  // The TCP/IP port to listen on
  private static final int PORT = config.listenPort;
  ...
  private static final int DEFAULT_TEMP = config.defaultTemperature;
  
  
  // Music server data
  private static final int MUSIC_SERVER_SOCKET_TIMEOUT = config.musicSocketTimeout;
  private static final String MUSIC_SERVER_HOST_NAME = config.musicServerName;
  ...
  private static final int OCCUPIED_INTERVAL = config.occupiedInterval; 
  
  // The time in milliseconds to wait for a ping to check for phone or computer
  private static final int PING_TIMEOUT = config.pingTimeout;
  ...
  private static final int BACKGROUND_DELAY = config.backgroundDelay;
  ...
  private void execute(String cmd) throws IOException {
    ...
    if (cmd.equals(CMD_TV_ON)) {
      ...
      writeSwitch(temp[HEATING_TEMPERATURE_SENSOR] < reqtemp);
      ...
      success();
        ...
        sendEmail("Your plant needs watering", "Its soil moisture is " + (100 - humidity[PLANT_HUMIDITY_SENSOR-1]) + "%");
        success();
